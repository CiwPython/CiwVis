 ...
 */
var view_width = document.getElementById("boxplotsnd").clientWidth;
var view_height = document.getElementById("boxplotsnd").clientHeight;
var margin = {top: 50, bottom: 50, left: 50, right: 50};
var width = view_width - margin.left - margin.right;
var height = view_height - margin.top - margin.bottom;

var svg_bxpltnd = d3.select('#boxplotsnd')
    .append('svg')
    ...
    */
    ...
    bpndfileReader.onload = event => {
        ...
        for (nd = 1; nd < num_nodes+1; nd++) {
            ...
            bpdata.push(boxplotdata_nd(dta, nd));
        ...
        var bxplt = svg_bxpltnd.selectAll(".box")
            .data(bpdata);
        ...
        newbxplt.merge(bxplt)
            .append("rect")
            .attr("class", "box")
            .attr("x", function(d) {return x_scale((0.4 + (d.Node * 1.8))*box_width);})
            .attr("stroke", "#ff6600")
            .attr("fill", "#ff6600")
        ...
        newbxplt.merge(bxplt)
            ...
            .attr("fill", "#ff6600")
            .attr("r", x_scale(0.4))
            .attr("cx", function(d) {return x_scale((0.9 + (1.8*d.Node))*box_width);})
        ...
        newbxplt.merge(bxplt)
            ...
            .attr("fill", "none")
            .attr("stroke", "#ff6600")
            .attr("x1", function(d) {return x_scale((0.4 + (d.Node * 1.8))*box_width);})
            .attr("x2", function(d) {return x_scale((1.4 + (d.Node * 1.8))*box_width);})
        ...
        newbxplt.merge(bxplt)
            ...
            .attr("fill", "none")
            .attr("stroke", "#ff6600")
            .attr("x1", function(d) {return x_scale((0.9 + (1.8*d.Node))*box_width);})
            .attr("x2", function(d) {return x_scale((0.9 + (1.8*d.Node))*box_width);})
        ...
        newbxplt.merge(bxplt)
            ...
            .attr("fill", "none")
            .attr("stroke", "#ff6600")
            .attr("x1", function(d) {return x_scale((0.7 + (1.8*d.Node))*box_width);})
            .attr("x2", function(d) {return x_scale((1.1 + (1.8*d.Node))*box_width);})
            .attr("y1", function(d) {return y_scale(d.tailmin)})
        ...
        newbxplt.merge(bxplt)
            ...
            .on("mousemove", function(d){
                svg_bxpltnd.select("#tooltip")
                    .select('rect')
                        .attr('x', d3.mouse(this)[0]-65)
                        .attr('y', d3.mouse(this)[1]-150);
                svg_bxpltnd.select("#tooltip")
                    .select('text').select('#bxnd_min')
                        .attr('x', d3.mouse(this)[0]-55)
                        .attr('y', d3.mouse(this)[1]-130);
                svg_bxpltnd.select("#tooltip")
                    .select('text').select('#bxnd_max')
                        .attr('x', d3.mouse(this)[0]-55)
                        .attr('y', d3.mouse(this)[1]-110);
                svg_bxpltnd.select("#tooltip")
                    .select('text').select('#bxnd_q25')
                        .attr('x', d3.mouse(this)[0]-55)
                        .attr('y', d3.mouse(this)[1]-90);
                svg_bxpltnd.select("#tooltip")
                    .select('text').select('#bxnd_q50')
                        .attr('x', d3.mouse(this)[0]-55)
                        .attr('y', d3.mouse(this)[1]-70);
                svg_bxpltnd.select("#tooltip")
                    .select('text').select('#bxnd_q75')
                        .attr('x', d3.mouse(this)[0]-55)
                        .attr('y', d3.mouse(this)[1]-50);
                svg_bxpltnd.select('#tooltip')
                    .select('text').select('#bxnd_mean')
                        .attr('x', d3.mouse(this)[0]-55)
                        .attr('y', d3.mouse(this)[1]-30);
            })
            .on("mouseout", function(d) {
                d3.select(this).attr("fill", "#ff6600");
                svg_bxpltnd.select("#tooltip").remove();
            });

        svg_bxpltnd.selectAll(".y.axis")
            .transition(t)
            .call(y_axis);
        svg_bxpltnd.append("g")
            .attr("transform", "translate(0," + height + ")")
